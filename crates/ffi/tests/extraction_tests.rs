use ffi::{DependencyConfig, extract_crate_spec};
    ...
    match result {
        Ok(spec) => {
            ...
            for item in &spec.items {
                let kind = match item {
                    ffi::PublicItem::Function { .. } => "Function",
                    ffi::PublicItem::Method { .. } => "Method",
                    ffi::PublicItem::AssocFunction { .. } => "AssocFunction",
                    ffi::PublicItem::Const { .. } => "Const",
                    ffi::PublicItem::Static { .. } => "Static",
                    ffi::PublicItem::Struct { .. } => "Struct",
                    ffi::PublicItem::Enum { .. } => "Enum",
                    ffi::PublicItem::TypeAlias { .. } => "TypeAlias",
                    ffi::PublicItem::Module { .. } => "Module",
                    ffi::PublicItem::Trait { .. } => "Trait",
                };
    ...
    match result {
        Ok(spec) => {
            ...
            let async_functions: Vec<_> = spec
                ...
                .iter()
                .filter_map(|item| {
                    if let ffi::PublicItem::Function { sig, .. } = item {
...
fn test_extract_reqwest() {
    ...
    match result {
        Ok(spec) => {
            ...
            for item in &spec.items {
                if let ffi::PublicItem::Struct { name, .. } = item
                    && name == "Client"
                {
                    println!("\n   Found Client struct!");
                }
            ...
            let method_count = spec
                .items
                .iter()
                .filter(|item| matches!(item, ffi::PublicItem::Method { .. }))
                .count();
...
";
    ...
    match result {
        Ok(spec) => {
            ...
            for item in &spec.items {
                match item {
                    ffi::PublicItem::Function { sig, .. } => {
                        println!("   - Function: {}", sig.name)
                    }
                    ffi::PublicItem::Method { sig, .. } => println!("   - Method: {}", sig.name),
                    ffi::PublicItem::AssocFunction { sig, .. } => {
                        println!("   - AssocFunction: {}", sig.name)
                    }
                    ffi::PublicItem::Const { name, .. } => println!("   - Const: {}", name),
                    ffi::PublicItem::Static { name, .. } => println!("   - Static: {}", name),
                    ffi::PublicItem::Struct { name, .. } => println!("   - Struct: {}", name),
                    ffi::PublicItem::Enum { name, .. } => println!("   - Enum: {}", name),
                    ffi::PublicItem::TypeAlias { name, .. } => println!("   - TypeAlias: {}", name),
                    ffi::PublicItem::Module { name, .. } => println!("   - Module: {}", name),
                    ffi::PublicItem::Trait { name, .. } => println!("   - Trait: {}", name),
                }
            }

            let has_point_struct = spec.items.iter().any(
                |item| matches!(item, ffi::PublicItem::Struct { name, .. } if name == "Point"),
            );
            let has_shape_enum = spec
                .items
                .iter()
                .any(|item| matches!(item, ffi::PublicItem::Enum { name, .. } if name == "Shape"));
            let has_add_function = spec.items.iter().any(
                |item| matches!(item, ffi::PublicItem::Function { sig, .. } if sig.name == "add"),
            );
            let has_pi_const = spec
                .items
                .iter()
                .any(|item| matches!(item, ffi::PublicItem::Const { name, .. } if name == "PI"));
            let has_new_method = spec.items.iter().any(|item| {
                matches!(item, ffi::PublicItem::AssocFunction { sig, .. } if sig.name == "new")
            });
            let has_distance_method = spec.items.iter().any(|item| {
                matches!(item, ffi::PublicItem::Method { sig, .. } if sig.name == "distance")
            });
            let has_nested_function = spec.items.iter().any(|item| {
                matches!(
                    item,
                    ffi::PublicItem::Function { sig, path, .. }
